...
### 02. 프로젝트 1-1 - 뉴스 수집 후, 텍스트 데이터 분석
 * 목적 : 정말로 코로나와 백신 관련 뉴스는 한쪽으로 편향되어 있는 것일까?에 대한 확인
 * 방법 : 
   * 01 네이버 뉴스의 데이터 수집
   * 02 네이버 뉴스의 제목 관련 키워드 분석 후, 긍정 부정 단어 빈도 분석(우선 구글 번역 이용)
   * 03 단어 빈도를 통해 어떠한지 1차 판단

 * 내용
   * 01 네이버 뉴스에서 백신 부작용 검색
   * 02 데이터를 파일로 정리
   * 03 간단한 텍스트 분석해 보기
